...
-- Ensure generate_prefixed_id function exists (defined in migration 0, may be missing on some envs)
...
CREATE INDEX IF NOT EXISTS idx_quote_feedback_quote ON quote_feedback(quote_id);
CREATE INDEX IF NOT EXISTS idx_quote_feedback_tenant ON quote_feedback(tenant_id);
CREATE INDEX IF NOT EXISTS idx_quote_feedback_status ON quote_feedback(tenant_id, status) WHERE status = 'pending';
...
CREATE TABLE IF NOT EXISTS quote_amendments (
  ...
  feedback_id UUID REFERENCES quote_feedback(id),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_quote_amendments_quote ON quote_amendments(quote_id);
CREATE INDEX IF NOT EXISTS idx_quote_amendments_tenant ON quote_amendments(tenant_id);
CREATE INDEX IF NOT EXISTS idx_quote_amendments_tenant_created ON quote_amendments(tenant_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_learning_context_tenant ON tenant_learning_context(tenant_id);
CREATE INDEX IF NOT EXISTS idx_learning_context_service ON tenant_learning_context(service_id);
